...
if (-not $codeMatch.Success) { throw 'The complete production installer Code section was not found.' }
$productionCode = $codeMatch.Groups['code'].Value
$productionIncludes = @{
    '#include "{{SOURCE_DIR}}\prerequisites\vc_runtime_code.iss"' = [IO.File]::ReadAllText((Join-Path $repository 'windows\packaging\exe\vc_runtime_code.iss'))
    '#include "{{SOURCE_DIR}}\prerequisites\install_integrity_code.iss"' = $source
}
foreach ($entry in $productionIncludes.GetEnumerator()) {
    if ([regex]::Matches($productionCode, [regex]::Escape($entry.Key)).Count -ne 1) {
        throw "The production Code include was not found exactly once: $($entry.Key)"
    }
    $productionCode = $productionCode.Replace($entry.Key, $entry.Value)
}
if ($productionCode.Contains('{{') -or $productionCode -match '(?m)^#include') {
    throw 'The complete production installer Code contains an unresolved template or include.'
}
$allMessages = @($template -split '\r?\n' | Where-Object { $_ -match '^(?:InstallIntegrity\w+|VcRuntime\w+|HelperStopFailed)=' }) -join "`r`n"
$compiledProduction = [Collections.Generic.List[string]]::new()
...
[Setup]
AppId=FwILCompileOnly
AppName=FengWo Complete Installer Code Compile Check
AppVersion=1.0
DefaultDirName=$caseRoot\compile-only
...
OutputBaseFilename=production-code-$architecture
[CustomMessages]
$allMessages
[Files]
Source: "$payload"; DestName: "vc_redist.exe"; Flags: dontcopy
[Code]
$productionCode
"@
    $fullFixturePath = Join-Path $OutputDirectory "production-code-$architecture.iss"
    [IO.File]::WriteAllText($fullFixturePath, $fullFixture, [Text.UTF8Encoding]::new($true))
    & $InnoCompiler /Q $fullFixturePath | Write-Host
    if ($LASTEXITCODE -ne 0) { throw "Complete production installer Code compilation failed: $architecture" }
    $compiledProduction.Add($architecture)
    Write-Host "Complete production installer Code compiled: $architecture"
}
@{ architectures=@($compiledProduction); combined_production_code=$true; executed=$false } | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath (Join-Path $OutputDirectory 'production-code-compile.json') -Encoding utf8
