...
void main() {
  ...
  test(
    'Android install permission retry keeps verified file and progress',
    () async {
      var requests = 0;
      var launches = 0;
      final providers = container(
      ...
      await notifier.install();
      final permission = providers.read(appUpdateDownloadProvider);
      expect(
        permission.failure,
        AppUpdateDownloadFailure.installPermissionRequired,
      );
      expect(permission.filePath, path);
      expect(permission.receivedBytes, bytes.length);
      expect(permission.installerLaunched, false);
      await notifier.install();
      expect(requests, 1);
      expect(launches, 2);
      expect(providers.read(appUpdateDownloadProvider).installerLaunched, true);
  ...
  test(
    'cancelled Android installer keeps package for explicit retry',
    () async {
      ...
      await started.future;
      expect(providers.read(appUpdateDownloadProvider).isBusy, true);
      await notifier.install();
      expect(launches, 1);
      result.completeError(
        const AppUpdateDownloadException(
          AppUpdateDownloadFailure.installCancelled,
        ),
      );
      await installing;
      final cancelled = providers.read(appUpdateDownloadProvider);
      expect(cancelled.failure, AppUpdateDownloadFailure.installCancelled);
      expect(cancelled.filePath, path);
      expect(cancelled.isBusy, false);
      expect(cancelled.installerLaunched, false);
      expect(requests, 1);
      expect(launches, 1);
      await notifier.install();
      expect(requests, 1);
