...
void main() {
  ...
  for (final previousAutoPreference in [false, true]) {
    testWidgets(
      'Android startup only updates badges with legacy preference $previousAutoPreference',
      (tester) async {
        var calls = 0;
        final container = _container(
          _service(
    ...
    (tester) async {
      ...
      final downloadService = AppUpdateDownloadService(
        ...
        transport: (uri, _) async {
          expect(uri, Uri.parse('https://update.example/FengWo.apk'));
          downloads++;
          started.complete();
      ...
      ((manifest['packages'] as Map)['android-arm64-v8a'] as Map)['sha256'] =
      ...
      try {
        ...
        expect(launchedPaths.single, endsWith('.apk'));
        expect(downloads, 1);
        await tester.tap(
...
ProviderContainer _container(
  ...
  return ProviderContainer(
    overrides: [
      ...
      appUpdatePlatformProvider.overrideWithValue('android'),
...
AppUpdateRelease _release() => AppUpdateRelease(
  ...
  version: '1.0.5',
  downloadUri: Uri.parse('https://update.example/FengWo.apk'),
...
Map<String, Object?> _manifest() => {
  ...
  'packages': {
    'android-arm64-v8a': {
      'enabled': true,
      'version': '1.0.5',
      'downloadUrl': 'https://update.example/FengWo.apk',
      'releaseNotesHtml': '<p>Update</p>',
    },
  },
};
