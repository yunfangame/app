...
void main() {
  ...
  group('getApplicationLocale', () {
    test('defaults to Simplified Chinese on Android', () {
      expect(
        utils.getApplicationLocale(null, isAndroid: true),
        const Locale('zh', 'CN'),
      );
    });

    test('keeps following the system locale outside Android', () {
      expect(utils.getApplicationLocale(null, isAndroid: false), isNull);
    });

    test('keeps an explicit language selection on Android', () {
      expect(
        utils.getApplicationLocale('en', isAndroid: true),
        const Locale('en'),
      );
    });
  });
