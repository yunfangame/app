...
function FwIntegritySecurityText(Descriptor: THandle; Revision, Information: Cardinal;
  var Text: THandle; var TextLength: Cardinal): Integer;
...
begin
  ...
  try
    ...
    try
      ...
      LabelLength := FwIntegrityTextLength(Text);
      if (LabelLength < 0) or (LabelLength > 65535) then
      begin
        Result := FwIntegrityFailure('Access', Path, 'invalid label length');
        Exit;
      end;
      SetLength(LabelText, LabelLength + 1);
      FwIntegrityCopyText(LabelText, Text, LabelLength + 1);
