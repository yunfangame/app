...
typedef XboardAnnouncementRequest = void Function({required bool automatic});
...
class GlobalState {
  ...
  void requestXboardAnnouncementAutoPrompt() {
    _xboardAnnouncementPromptPending = true;
    showXboardAnnouncements?.call(automatic: true);
  }

  bool consumeXboardAnnouncementAutoPrompt() {
    final pending = _xboardAnnouncementPromptPending;
  ...
  bool isActiveXboardSession(XboardLoginResult session, int revision) {
    return revision == _xboardSessionRevision &&
        identical(session, xboardSession);
  }
  ...
  Future<ProviderContainer> _initData(int version) async {
    ...
    await AppLocalizations.load(
      utils.getApplicationLocale(
            config.appSettingProps.locale,
